...
def _validate_condition_ast(node):
    """Validate that the parsed condition AST only contains a safe subset of
    Python expressions (boolean operations, comparisons, names, literals, and
    calls to the Version helper)."""
    ...
    allowed_bin_ops = (ast.Eq, ast.NotEq, ast.Lt, ast.LtE, ast.Gt, ast.GtE)
    ...
    def _check(n):
        ...
        if isinstance(n, ast.BoolOp):
            if not isinstance(n.op, allowed_bool_ops):
                raise GypError(f"Unsupported boolean operator in condition: {ast.dump(n.op)}")
            for v in n.values:
                _check(v)
            return
        if isinstance(n, ast.UnaryOp):
            if not isinstance(n.op, allowed_unary_ops):
                raise GypError(f"Unsupported unary operator in condition: {ast.dump(n.op)}")
            _check(n.operand)
            return
        if isinstance(n, ast.BinOp):
            # Only allow comparisons implemented as binary ops if any appear;
            # arithmetic inside conditions is not required for gyp.
            if not isinstance(n.op, allowed_bin_ops):
                raise GypError(f"Unsupported binary operator in condition: {ast.dump(n.op)}")
            _check(n.left)
            _check(n.right)
            return
        ...
        if isinstance(n, ast.Name):
            # Names are resolved from the variables mapping at evaluation time.
            return
        if isinstance(n, ast.Constant):
            # Literal values (str, int, bool, etc.) are allowed.
            return
        if isinstance(n, ast.Call):
            # Only allow calling the Version helper.
            if not isinstance(n.func, ast.Name) or n.func.id not in ("Version", "v"):
                raise GypError("Only calls to Version(...) are allowed in conditions.")
            for arg in n.args:
                _check(arg)
        ...
        if not isinstance(n, allowed_expr_nodes):
            raise GypError(f"Unsupported syntax in condition: {ast.dump(n)}")

    _check(node)


def _eval_condition_ast(node, variables):
    """Evaluate a previously validated condition AST using the provided
    variables mapping and Version helper."""
    ...
    def _eval(n):
        ...
        if isinstance(n, ast.Constant):
            return n.value
        if isinstance(n, ast.Name):
            return variables.get(n.id, False)
        if isinstance(n, ast.BoolOp):
            vals = [_eval(v) for v in n.values]
            if isinstance(n.op, ast.And):
                result = True
                for v in vals:
                    result = bool(result and v)
                    if not result:
                        break
                return result
            if isinstance(n.op, ast.Or):
                result = False
                for v in vals:
                    result = bool(result or v)
                    if result:
                        break
                return result
            raise GypError("Unsupported boolean operator in condition.")
        if isinstance(n, ast.UnaryOp):
            operand = _eval(n.operand)
            if isinstance(n.op, ast.Not):
                return not bool(operand)
            raise GypError("Unsupported unary operator in condition.")
        if isinstance(n, ast.Compare):
            left = _eval(n.left)
            result = True
            current = left
            for op, comparator in zip(n.ops, n.comparators):
                ...
                if isinstance(op, ast.Eq):
                    ...
                    raise GypError("Unsupported comparison operator in condition.")
                if not ok:
                    return False
                current = right
            return result
        if isinstance(n, ast.Call):
            if isinstance(n.func, ast.Name) and n.func.id in ("Version", "v"):
                args = [_eval(a) for a in n.args]
                kwargs = {kw.arg: _eval(kw.value) for kw in n.keywords}
                return Version(*args, **kwargs)
            raise GypError("Only calls to Version(...) are allowed in conditions.")
        raise GypError(f"Unsupported expression in condition: {ast.dump(n)}")
...
def EvalSingleCondition(cond_expr, true_dict, false_dict, phase, variables, build_file):
  ...
  otherwise."""
    ...
    try:
        # Cache the parsed and validated AST for this condition string.
        if cond_expr_expanded in cached_conditions_asts:
            cond_ast = cached_conditions_asts[cond_expr_expanded]
        else:
            parsed = ast.parse(str(cond_expr_expanded), "<string>", mode="eval")
            # Validate that the AST is within the allowed subset.
            _validate_condition_ast(parsed)
            cond_ast = parsed
            cached_conditions_asts[cond_expr_expanded] = cond_ast

        # Safely evaluate the validated AST against the provided variables.
        result = _eval_condition_ast(cond_ast, variables)
        if result:
            return true_dict
        return false_dict
    except SyntaxError as e:
